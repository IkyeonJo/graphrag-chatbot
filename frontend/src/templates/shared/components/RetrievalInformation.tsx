    ...
    <Box className='n-bg-palette-neutral-bg-weak p-4'>
      <Flex flexDirection='row' className='flex flex-row p-6 items-center'>
        ...
        <Box className='flex flex-col'>
          <Typography variant='h2'>검색 정보</Typography>
          <Typography className='mb-2' variant='body-medium'>
            이 응답을 생성하기 위해 <span className='font-bold italic'>{model}</span> 모델을 사용했습니다.
            <Typography className='pl-1 italic' variant='body-small'>
              <ClockIconOutline className='w-4 h-4 inline-block mb-1' /> {timeTaken / 1000}초
            </Typography>
          </Typography>
        </Box>
      </Flex>
      <Box className='button-container' sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
        ...
        >
          ...
          >
            <IconButton className='n-size-token-7' ariaLabel='화면에 맞추기' onClick={fitNodes}>
              <FitToScreenIcon />
            </IconButton>
            <IconButton className='n-size-token-7' ariaLabel='확대 초기화' onClick={resetZoom}>
              <ResetZoomIcon />
            </IconButton>
          </Flex>
          {loading ? (
          ...
          <Box className='max-w-[300px]'>
            <Drawer isCloseable={true} isExpanded={isExpanded} position="left" type="overlay" onExpandedChange={() => {
              ...
              <Drawer.Header>노드 상세</Drawer.Header>
              <Drawer.Body className='max-w-[300px]'>
                {expandedNode?.captions[0]?.value?.includes('Chunk') ? (<Typography variant='h5'>청크 텍스트: </Typography>) : expandedNode?.captions[0]?.value?.includes('Document') ? (<Typography variant='h5'>문서: </Typography>) : (<Typography variant='h5'>엔티티 설명: </Typography>)}
                <ReactMarkdown>
