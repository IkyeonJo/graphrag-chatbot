            ...
            <div className='flex grow-0 gap-x-1 w-max items-center pr-3'>
              {useNeo4jConnect ? (
                <Switch
                  ...
                  disabled={false}
                  fluid={true}
                  label={`Neo4j ${connectNeo4j ? '연결됨' : '연결'}`}
                  labelBefore={true}
                />
              ) : null}
              <IconButton aria-label='다크 모드 전환' isClean size='large' onClick={toggleColorMode} ariaLabel=''>
                {themeMode === 'dark' ? (
                  <span role='img' aria-label='해'>
                    <SunIconOutline />
                  </span>
                ) : (
                  <span role='img' aria-label='달'>
                    <MoonIconOutline />
                  </span>
                )}
              </IconButton>
              <IconButton onClick={() => handleURLClick("https://neo4j.com/labs/neo4j-needle-starterkit/")} className='hidden md:inline-flex' aria-label='도움말' isClean size='large' ariaLabel=''>
